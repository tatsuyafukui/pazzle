...
const UploadForm: React.FC<IProps> = props => {
  ...
  const changeFileHandler = (event: React.FormEvent<HTMLInputElement>) => {
    ...
    reader.addEventListener('load', (event: any) => {
      image.addEventListener('load', () => {
        ...
        function scaling( _v: any ) {        // スライダーが変った
          v =  _v * 0.01;
          draw_canvas( ix, iy )       // 画像更新
        }

        function draw_canvas( _x: any, _y: any ){     // 画像更新
        ...
        let sy = 0;
        cvs.ontouchstart =
          cvs.onmousedown = function ( _ev: any ){     // canvas ドラッグ開始位置
            mouse_down = true;
            sx = _ev.pageX;
            sy = _ev.pageY;
        ...
        cvs.ontouchend =
          cvs.onmouseout =
            cvs.onmouseup = function ( _ev : any){       // canvas ドラッグ終了位置
        ...
        cvs.ontouchmove =
          cvs.onmousemove = function ( _ev: any ){     // canvas ドラッグ中
            if (!mouse_down) return
            draw_canvas( ix + (sx-_ev.pageX)/v, iy + (sy-_ev.pageY)/v );
            return false // イベントを伝搬しない
          }
  ...
  return (
    <Modal
      ...
      onClick={() => {dispatch(toggleUploadModal(props.hasUploadModal))}}
    >
      <div onClick={() => {dispatch(toggleUploadModal(props.hasUploadModal))}}>
        <FontAwesomeIcon
          icon={faTimesCircle}
